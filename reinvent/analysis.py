...
if __name__ == '__main__':
    ...
    for fname in tqdm(fnames):
        ...
        results_per_gen.append(pd.DataFrame(new_df))
    ...
    results_per_gen = results_per_gen.rename(columns={'generation': 'Generation', 
        'best_fitness': f'{FLAGS.target} Docking Score', 'run_type': 'Run type', 
        'avg_fitness': f'Average {FLAGS.target} score in population',
        'is_stereo_percent': 'Percent of stereosmiles'})
    sns.lineplot(data=results_per_gen, x='Generation', y=f'{FLAGS.target} Docking Score', hue='Run type', palette=CMAP)
