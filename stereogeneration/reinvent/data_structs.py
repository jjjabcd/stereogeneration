...
class Vocabulary(object):
    ...
    def encode(self, char_list):
        ...
        # char_list = replace_halogen(char_list)
        # char_list = re.split('(\[[^\[\]]{1,6}\])', char_list) 
...
class Experience(object):
    ...
    def sample(self, n):
        ...
        if len(self.memory)<n:
            ...
            scores = np.array(scores) + 1.0         # since score range from -1 to 1
    ...
    def print_memory(self, path, fitness_dict = None):
        """Prints the memory."""
        print("\n" + "*" * 80 + "\n")
        print("         Best recorded SMILES: \n")
        print("Score     Fitness     Prior log P     SMILES\n")
        with open(path, 'w') as f:
            f.write("SMILES Score Fitness PriorLogP\n")
            for i, exp in enumerate(self.memory[:100]):
                if i < 50:
                    if fitness_dict is not None:
                        print(f"{exp[1]:4.2f}     {fitness_dict[exp[0]]:4.2f}        {exp[2]:6.2f}          {exp[0]}")
                        f.write(f"{exp[0]} {exp[1]:4.2f} {fitness_dict[exp[0]]:4.2f} {exp[2]:6.2f}\n")
                    else:
...
def construct_vocabulary(smiles_list, path='data/Voc'):
