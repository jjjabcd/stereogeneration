...
def mutate_sf(sf_chars, alphabet, num_sample_frags, base_alphabet = None, weights = None):
    ...
    if base_alphabet is None:
        base_alphabet = list(group_selfies.get_semantic_robust_alphabet())
        weights = None
...
def mutate_smiles(
    ...
    for _ in range(num_random_samples):
        ...
        # selfies_ls = group_grammar.full_encoder(Chem.MolFromSmiles(random_smile))
        selfies_ls = encode_func(Chem.MolFromSmiles(random_smile))
        selfies_ls_chars = list(group_selfies.split_selfies(selfies_ls))
        ...
        for sf in mutated_sf:
            try: 
                # mutated_mol = group_grammar.decoder(sf)
                mutated_mol = decode_func(sf)
