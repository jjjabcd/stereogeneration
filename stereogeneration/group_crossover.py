...
def obtain_path(starting_smile, target_smile, encode_func, decode_func):
    """
    ...
    """
    starting_selfie = encode_func(Chem.MolFromSmiles(starting_smile))
    target_selfie = encode_func(Chem.MolFromSmiles(target_smile))

    starting_selfie_chars = list(group_selfies.split_selfies(starting_selfie))
    target_selfie_char = list(group_selfies.split_selfies(target_selfie))
    ...
    path_smiles = [Chem.MolToSmiles(decode_func(x), isomericSmiles=True) for x in paths_selfies]

    return path_smiles


def perform_crossover(comb_smi, num_random_samples, encode_func, decode_func, stereo=True):
    """
    ...
    for smi_1 in randomized_smile_orderings_a:
        for smi_2 in randomized_smile_orderings_b:
            for item in obtain_path(smi_1, smi_2, encode_func, decode_func):
...
def crossover_smiles(smiles_join, crossover_num_random_samples, encode_func, decode_func, stereo=True):
    """
